...
//  WorkOutApps
//
//  Created by Daniil Zolotarev on 7.05.24.
//
...
final class TimerView: WABaseInfoView {
    ...
    private let remaningTimeLable: UILabel = {
        let lable = UILabel()
    ...
    func configure(with duration: Double, progress: Double) {
        ...
        let roundedPercent = Int(round(persent * 100))
        
        elapsedTimeValueLable.text = getDiplayedString(from: Int(tempCurrentValue))
        remainingTimeValueLable.text = getDiplayedString(from: Int(duration) - Int(tempCurrentValue))
        ...
        progressView.drawProgress(with: CGFloat(persent))
    }
...
private extension TimerView {
    func getDiplayedString(from value: Int) -> String {
